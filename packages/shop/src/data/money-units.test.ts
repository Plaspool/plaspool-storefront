...
describe("sizesFrom — the one place minor units become whole naira", () => {
  ...
  it("converts the API's minor units to the `priceNaira` the UI formats", () => {
    const [size] = sizesFrom([variant({})]);
    expect(size.priceNaira).toBe(18500);
    expect(formatNaira(size.priceNaira)).toBe("₦18,500");
  });
  ...
  it("keeps the cheapest priced variant for a repeated weight", () => {
    ...
    expect(sizes[0].priceNaira).toBe(17000);
  });
  ...
  it("sorts ascending by price", () => {
    ...
    expect(sizes.map((s) => s.priceNaira)).toEqual([6500, 18500]);
  });

  it("rounds, so no float reaches a price", () => {
    const [size] = sizesFrom([variant({ price: { amount: 99999, currency: "NGN" } })]);
    expect(size.priceNaira).toBe(1000);
    expect(Number.isInteger(size.priceNaira)).toBe(true);
  });
});
